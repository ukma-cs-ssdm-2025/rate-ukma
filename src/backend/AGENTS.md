...
Conventions: [docs/testing/backend-tests.md](../../docs/testing/backend-tests.md) — read it before writing a test. The traps that cost the most time:

- Factories are fixtures, not imports: `course_factory`, `rating_factory`, … are generated by `register()` in `conftest.py`, so there is no definition to grep. List them with `grep -n '^register(' conftest.py`.
- `token_client.user` has no `Student` row. Anything behind `require_student` or an enrolment check needs `student_factory(user=token_client.user)` plus `enrollment_factory(...)`, or the happy path returns `403` and looks like broken auth.
- Unauthenticated requests get `403`, not `401` — `SessionAuthentication` sends no `WWW-Authenticate`.
- A database test carries `@pytest.mark.django_db` and `@pytest.mark.integration`; there is no module-level `pytestmark` and nothing derives the second from the first.
- Run from `src/backend`: `uv run pytest <file> -q`, after `uv sync --extra test` and a `.env` copied from `src/.env.sample`.
