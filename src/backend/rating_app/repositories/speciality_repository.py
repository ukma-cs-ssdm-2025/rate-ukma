...
class SpecialityRepository(IRepository[Speciality]):
    def get_all(self) -> list[Speciality]:  # type: ignore[override]
        return list(Speciality.objects.select_related("faculty").all())

    def get_by_id(self, speciality_id: str) -> Speciality:  # type: ignore[override]
    ...
    def get_or_create(self, *, name: str, faculty: "Faculty") -> tuple[Speciality, bool]:  # type: ignore[override]
    ...
    def update(self, speciality: Speciality, **speciality_data) -> Speciality:  # type: ignore[override]
        for field, value in speciality_data.items():
            setattr(speciality, field, value)
        speciality.save()
        return speciality

    def delete(self, speciality: Speciality) -> None:  # type: ignore[override]
        speciality.delete()

    def filter(self, *args: Any, **kwargs: Any) -> list[Speciality]:  # type: ignore[override]
